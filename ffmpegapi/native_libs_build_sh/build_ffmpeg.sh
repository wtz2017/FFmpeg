...
OPENSSL_BUILD_DIR=/home/ubuntu/openssl/openssl-1.1.1c/android
...
function build {
    SSL_EXTRA_CFLAGS="-I${OPENSSL_BUILD_DIR}/${ABI}/include"
    SSL_EXTRA_LDFLAGS="-L${OPENSSL_BUILD_DIR}/${ABI}/lib"
    ...
    --sysroot=${SYSROOT} \
    --prefix=${PREFIX} \
    --extra-cflags="-Os -fpic ${ADDI_CFLAGS} ${SSL_EXTRA_CFLAGS}" \
    --extra-ldflags="${ADDI_LDFLAGS} ${SSL_EXTRA_LDFLAGS}" \
    ...
    --disable-programs \
    --disable-doc \
    $ADDITIONAL_CONFIGURE_FLAG || terminate "Configure ffmpeg for ${ABI} failed!"
    echo -e "====== configure finished ======\n"

    make clean
    echo -e "====== clean finished ======\n"

    make -j4 || terminate "Make ffmpeg for ${ABI} failed!"
